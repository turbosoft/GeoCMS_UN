...
function inputGeometryShape(type) {
	...
	if(type==1) {
		canvas_element.mousedown(function(e) {
			...
			geometry_point_arr_1 = null; 
			geometry_point_arr_2 = null; 
			geometry_point_arr_1 = new Array(); 
			geometry_point_arr_2 = new Array();
			geometry_point_arr_1.push(geometry_click_move_point_x); 
			geometry_point_arr_2.push(geometry_click_move_point_y);
		});
...
function createGeometry(type) {
	...
	if(type==1 || type==2) 
	{
		if(geometry_point_arr_1[0] < geometry_point_arr_1[1]) 
		{ 
			min_x = geometry_point_arr_1[0]; max_x = geometry_point_arr_1[1]; 
		}
		else 
		{ 
			min_x = geometry_point_arr_1[1]; max_x = geometry_point_arr_1[0]; 
		}
	...
	if(type==5)
	{
 		left_str = $('#image_write_canvas').css('left'); 
 		top_str = $('#image_write_canvas').css('top');
	}
	else
	{
		left_str = $('#image_write_canvas_div').css('left'); 
		top_str = $('#image_write_canvas_div').css('top');
	}
...
function insertTableObject(data_arr) {
	var html_text = "";
	html_text += "<tr id='obj_tr"+data_arr[0]+"' bgcolor='#FF0000' style='font-size:12px; height:18px; color:rgb(21, 74, 119);'>";
...
function detectImage(){
	...
	$.ajax({
		url: 'http://deepapi.u-gis.net/?command=detect&model=mscoco&img=http://localhost:8125/GeoCMS/upload/GeoPhoto/'+file_url,
		dataType: 'json',
		beforeSend:function(){
			$.loadingBlockShow();
		},
		success: function (result) 
		{
			...
			else 
			{
				...
				var canvasWidth = $('#image_write_canvas').width();
				var canvasHeight = $('#image_write_canvas').height();
...
<!-- <div id='exif_dialog' style='position:absolute; left:800px; top:310px; width:300px; height:200px; border:1px solid #999999; display:block; font-size:13px;'> -->
	...
	<div id="detectBtn" class="smallGreyBtn" onclick="detectImage();" style="width: 60px;height: 20px;float: right;border-radius:5px;text-align: center;position: absolute;top: 511px;left: 740px;cursor: pointer;font-size: 13px;">Detect</div>
	<textarea id="text" name="text" style="width: 60px;height: 20px; float: right;border-radius:5px;text-align: center;position: absolute;top: 511px;cursor: pointer;"></textarea>
<!-- </div> -->
		...
		<div align="center" style='display:table-cell; vertical-align:middle;'>
			<table id='geometry_table' border="0" style="width: 100%;">
				<tr>
					...
					<input id="geometry_line_color" type="text" class="iColorPicker" value="#FF0000" style="width:80px; margin-left: 8px;"/>
					&nbsp;&nbsp;&nbsp;

...
// 	contentStr += '<br><label>KakaoTalk_20161121_174514428.jpg</label>';
	...
	marker.setIconImage(tmpMarkerIcon);
	
	// 마커의 z-Index 설정
    marker.setZindex(3);
	
    marker.id = p[3]+'_'+p[4]+'_'+p[7]+'_'+p[8],			//p[3]: index, p[4]: kind(GeoPhoto, GeoVideo), p[7]: use_id, p[8]: project make user id
    ...
    marker.events.register('click', marker, function(event){
		...
		if(kindStr == 'GeoPhoto'){
			...
			videoViewer(this.mtitle2.split('/')[0], this.mtitle2.split('/')[1], this.id.split("_")[2], this.id.split("_")[0],this.id.split("_")[3]);
		}
    });
    ...
    marker.events.register('mouseover', marker, function(event){
    	...
    	var contentStr = "<img class='round' src='" + ftpBaseUrl() + "/" + kindStr +"/"+tmpThumbFileName1+"' width='200' height='200' hspace='10' vspace='10' style='border:2px solid #888888'/>";
...
// 		jAlert('좌표 정보가 없습니다. viewer로 이동합니다.','정보');
		jAlert('No coordinate information. Go to the viewer.','Info');
		if(kindStr == 'GeoPhoto'){
			if(projectImage == 1){
	...
	$.each(markerArr, function(idx, val){
		...
		if(oldMarkerData != null && oldMarkerData.length > 0 && val.id.trim() == oldMarkerData[3]+'_'+oldMarkerData[4]+'_'+oldMarkerData[7]+'_'+oldMarkerData[8]){
			val.setIconImage(tmpMarkerIcon);
			val.setZindex(3);
		}
		if(val.id == objId){
			this.setIconImage(tmpMarkerIconClk);
			val.setZindex(4);
			cnt = 1;
		}
...
// 	apiMap.setCenterAndZoom(lon, lat, 10);//화면중심점과 레벨로 이동 (초기 화면중심점과 레벨)
